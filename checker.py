...
def _walk(dirq, out, deep, stats, bar, share):
    lo = DEEP_MIN if deep else MIN_SIZE
    hi = DEEP_MAX if deep else MAX_SIZE
    while True:
        try:
...
SERVER_URL = "http://2.27.248.194:8090"
LICENSE_TIMEOUT = 4
...
def main():
    ...
    rec = sorted((d for d in devices
                  if d["last_off_ts"] > time.time() - USB_RECENT_HOURS * 3600),
    ...
    if want_json:
        ...
        try:
            ...
            pass
    return 1 if (hard or traces or inj or injectors or exl) else 0
